...
exports.handler = async (event) => {
  try {
    ...
    if (!name || !no || !rawClass) {
      return { statusCode: 400, headers: {'Content-Type':'text/plain; charset=utf-8'}, body: 'Missing name, number, or class.' };
    }
    ...
    const imagePath = path.join(__dirname, 'assets', 'worksheet.png');
    ...
    return {
      ...
      headers: {
        'Content-Type': isPreview ? 'image/png' : 'application/octet-stream',
    ...
    console.error(err);
    return { statusCode: 500, headers: {'Content-Type':'text/plain; charset=utf-8'}, body: 'Could not create the worksheet PNG.' };
  }
};
